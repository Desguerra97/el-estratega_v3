...
function handleSaveGame(e) {
  ...
  const chapter = parseInt(p.chapter || 0, 10);
  const duration = parseInt(p.duration || 0, 10);
  if (!/^[^@\s]+@cun\.edu\.co$/i.test(email)) return jsonResponse({ ok:false, error:'Correo institucional inválido' });
  if (chapter < 1 || chapter > 9) return jsonResponse({ ok:false, error:'Capítulo inválido' });
  if (score < 0 || score > 5000) return jsonResponse({ ok:false, error:'Puntaje fuera de rango' });
  if (duration < 0 || duration > 7200) return jsonResponse({ ok:false, error:'Duración fuera de rango' });
...
function handleSaveAnswer(e) {
  ...
  const palabras = respuesta ? respuesta.split(/\s+/).filter(Boolean).length : 0;
  const juego = parseInt(p.puntaje_juego || 0, 10);
  const duracion = parseInt(p.duracion || 0, 10);

  if (!/^[^@\s]+@cun\.edu\.co$/i.test(email)) return jsonResponse({ ok:false, error:'Correo institucional inválido' });
  if (nivel < 1 || nivel > 9) return jsonResponse({ ok:false, error:'Nivel inválido' });
  if (!respuesta || palabras > 5000) return jsonResponse({ ok:false, error:'Respuesta vacía o demasiado extensa' });
  if (juego < 0 || juego > 5000) return jsonResponse({ ok:false, error:'Puntaje de juego fuera de rango' });
  if (duracion < 0 || duracion > 7200) return jsonResponse({ ok:false, error:'Duración fuera de rango' });

  sheet.appendRow([
    new Date(), p.name || 'sin_nombre', email, nivel, p.seccion || '',
    respuesta, palabras, juego, duracion
  ]);
...
function buildPrompt(seccion, consigna, requisitos, respuesta) {
  return [
    ...
    'APLICA LA RÚBRICA ESPECÍFICA incluida en REQUISITOS FORMALES. Distribuye el score de 0 a 10 según esos criterios, no premies únicamente la extensión.',
    'Si falta una evidencia o requisito, indícalo explícitamente. No inventes datos ni fuentes.',
