...
(function () {
  ...
  let ctx, canvas;
  let level, onEnd;
  let player, tokens, enemies, walls, door, portals, rocks, books, hazards, checkpoints, questionGates, chests, boss, puzzleSwitches, clueState;
  let routeCells;
  ...
  function loadSprites() {
    const names = ['estratega','dementor','snake','rock','chest','diamond','spikes','cage','door','trap'];
    names.forEach(name => { const img = new Image(); img.src = `assets/sprites/${name}.svg`; SPRITES[name] = img; });
  }
  ...
  window.DiamondRush = {
    start(lvl, cb, gameConfig) {
      ...
      bossDefeated = false;
      checkpointRespawn = { gx: 2, gy: 2 };
      clueState = { found: 0, total: 0, texts: [] };
      puzzleSwitches = [];
      clueState = { found: 0, total: 0, texts: [] };
      ...
      player = {
        ...
        fromX: 2 * CELL, fromY: 2 * CELL,
        toX: 2 * CELL, toY: 2 * CELL
      };
      updateHUD();
      renderPowerBar();
      bindControls();
  ...
  function generateMap() {
    ...
    const puzzleCount = 1; // Un solo acertijo central: la puerta final.
    ...
    // Habitación inicial REAL: paredes físicas, una sola salida y sin amenazas.
    // El jugador puede observar el mapa y pensar antes de entrar al peligro.
    for (let x = 1; x <= 4; x++) {
      walls.add(x + ',1');
      walls.add(x + ',4');
    }
    ...
    walls.delete('4,3'); // única salida de la habitación segura
    walls.delete('2,2');
    walls.delete('3,2');
    walls.delete('2,3');
    walls.delete('3,3');
    ...
    // Rocas: desde el nivel 1 siempre hay rocas para resolver el acceso al cofre.
    const rockTarget = Math.max(2, Number(dif.rocas || 0));
    for (let r = 0; r < rockTarget; r++) {
    ...
    const wrongList = []; // Solo coleccionables positivos; el aprendizaje ocurre mediante preguntas.
    ...
    corrList.forEach((text, i) => {
      ...
      tokens.push({ gx: cell.x, gy: cell.y, text, correct: true, collected: false, glow: 0, bloqueado, quiz: null, unlockedByGroup: true });
    });
    ...
    books = []; // Durante el juego: solo preguntas, sin lecturas.

    // El recorrido se resuelve con rocas, cofres, trampas y una cerradura final.
    // No hay puertas secundarias: la única puerta importante es la salida.
    //      una sola cerradura conceptual abre la puerta.
    // Los cofres entregan pistas después de responder una pregunta.
    const puzzleQuizzes = level.quiz_bloqueo || [];
    const finalPuzzle = level.door_puzzle || (puzzleQuizzes[0] || { q: '¿Cuál es la mejor decisión estratégica?', opciones: ['Copiar a todos', 'Crear una ventaja sostenible', 'Esperar sin investigar'], correcta: 1 });
    const puzzleCell = { x: COLS - 4, y: ROWS - 3 };
    walls.delete(puzzleCell.x + ',' + puzzleCell.y);
    routeCells.add(puzzleCell.x + ',' + puzzleCell.y);
    puzzleSwitches.push({ gx: puzzleCell.x, gy: puzzleCell.y, quiz: finalPuzzle, active: false, index: 1 });
    door.puzzleRequired = 1;

    const clueList = level.chest_clues || puzzleQuizzes.slice(0, 2);
    clueState = { found: 0, total: Math.max(1, Math.min(2, clueList.length)), texts: [] };

    const hazardCount = Math.max(1, Math.min(2 + Math.floor((level.num || 1) / 2), 4));
    for (let i = 0; i < hazardCount; i++) {
      const cell = randomEmptyCell();
      if (cell) hazards.push({ gx: cell.x, gy: cell.y, type: i % 2 === 0 ? 'spikes' : 'fire', active: true, triggered: false, phase: Math.random() * Math.PI * 2, cageId: i });
    }

    // Cofres: desde el nivel 1, al menos uno. Su contenido es una pista conceptual.
    const chestCount = Math.max(1, Math.min(1 + Math.floor((level.num || 1) / 3), 3));
    const clueQuestions = level.chest_clues || qlist;
    for (let i = 0; i < chestCount; i++) {
      const cell = randomEmptyCell();
      if (cell) chests.push({ gx: cell.x, gy: cell.y, opened: false, quiz: clueQuestions.length ? clueQuestions[i % clueQuestions.length] : null, clue: (level.clues && level.clues[i]) || 'Busca una idea que conecte la pregunta con el problema del nivel.', reward: 'clue' });
    }

    // Cada cofre tiene al menos una roca bloqueando uno de sus accesos: hay que empujar para llegar.
    chests.forEach((c, idx) => {
      const candidates = [[-1,0],[0,-1],[1,0],[0,1]];
      let placedBlocker = false;
      for (const [dx,dy] of candidates) {
        const bx=c.gx+dx, by=c.gy+dy;
        if (bx<=1||bx>=COLS-2||by<=1||by>=ROWS-2) continue;
        if (walls.has(bx+','+by) || isStartSafeZone(bx,by)) continue;
        const existing=rocks.find(r=>r.gx===bx&&r.gy===by);
        if (existing) { placedBlocker=true; break; }
        if (!isOccupied(bx,by)) {
          rocks.push({gx:bx,gy:by,moving:false,moveT:0,fromX:bx*CELL,fromY:by*CELL,toX:bx*CELL,toY:by*CELL,px:bx*CELL,py:by*CELL});
          placedBlocker=true; break;
        }
      }
      if (!placedBlocker) {
        // No forzamos una roca imposible; el cofre seguirá siendo accesible por otro camino.
      }
    });
    ...
    for (let i = 0; i < Math.max(1, Number(dif.enemigos || 0)); i++) placeEnemy(i);
    // Cada enemigo nace junto a su propia trampa: al pisarla, la jaula se abre.
    enemies.filter(e => e.chase).forEach((e, i) => {
      e.caged = true; e.active = false; e.cageId = i % Math.max(1, hazards.length);
      const h = hazards[e.cageId];
      if (h) {
        const near = [[1,0],[-1,0],[0,1],[0,-1]].map(([dx,dy])=>({x:h.gx+dx,y:h.gy+dy}))
          .find(c=>c.x>1&&c.x<COLS-2&&c.y>1&&c.y<ROWS-2&&!isOccupied(c.x,c.y));
        if (near) { e.gx=near.x; e.gy=near.y; e.px=near.x*CELL; e.py=near.y*CELL; e.fromX=e.px; e.fromY=e.py; e.toX=e.px; e.toY=e.py; }
      }
    });
    boss = null; // El guardián final queda para una futura versión; la puerta es el reto principal.
  }
  ...
  function placeEnemy(idx) {
    const cell = randomEmptyCell();
    if (!cell) return;
    const type = idx % 2 === 0 ? 'dementor' : 'snake';
    enemies.push({
      gx: cell.x, gy: cell.y,
      ...
      speed: type === 'snake' ? 170 : 205,
      chase: true, active: false, caged: true
    });
  }
  ...
  function tryMove(dir) {
    ...
    if (nx === door.gx && ny === door.gy) {
      ...
      addMessage(left ? '🔒 Resuelve el acertijo de la puerta' : '🔒 La puerta espera tu respuesta', door.gx, door.gy, '#ff5c5c');
      return;
    }
  ...
  function onArriveCell() {
    ...
    // Token: los diamantes son opcionales; las decisiones importantes son los cofres y la puerta.
    // Token?
    for (const t of tokens) {
      if (t.collected) continue;
  ...
  function openPuzzleQuiz(puzzle) {
    ...
    const q = puzzle.quiz;
    const neededClues = Math.min(clueState.total, level.num >= 4 ? 2 : 1);
    if (clueState.found < neededClues) {
      quizActive = false;
      addMessage('🔒 Busca primero ' + neededClues + ' pista' + (neededClues > 1 ? 's' : '') + ' en los cofres', player.gx, player.gy, '#ffd94d');
      return;
    }
    if (!overlay || !q) { activatePuzzleSwitch(puzzle); return; }
    overlay.innerHTML = `<div class="quiz-modal door-puzzle"><div class="quiz-header">🚪 Acertijo de la puerta</div><div class="clue-strip">${clueState.texts.map((t,i)=>`<span>💡 ${escapeHtml(t)}</span>`).join('')}</div><div class="quiz-question">${escapeHtml(q.q)}</div><div class="quiz-options">${q.opciones.map((o,i)=>`<button class="quiz-opt" data-i="${i}">${escapeHtml(o)}</button>`).join('')}</div><p class="quiz-hint">Usa las pistas de los cofres para resolver el acertijo y abrir la puerta.</p></div>`;
  ...
  function activatePuzzleSwitch(puzzle) {
    ...
    addMessage('🚪 ¡ACERTIJO RESUELTO! LA PUERTA SE ABRE', puzzle.gx, puzzle.gy, '#ffd94d');
  ...
  function openChestQuiz(chest) {
    if (quizActive || chest.opened) return;
    quizActive = true;
    const overlay = document.getElementById('quiz-overlay');
    const q = chest.quiz;
    if (!overlay || !q) { chest.opened = true; clueState.found++; clueState.texts.push(chest.clue); closeModal(); return; }
    overlay.innerHTML = `<div class="quiz-modal chest-clue"><div class="quiz-header">🧰 Cofre de pista</div><div class="quiz-question">${escapeHtml(q.q)}</div><div class="quiz-options">${q.opciones.map((o,i)=>`<button class="quiz-opt" data-i="${i}">${escapeHtml(o)}</button>`).join('')}</div><p class="quiz-hint">Responde para abrir el cofre y descubrir una pista para la puerta.</p></div>`;
    overlay.style.display='flex';
    overlay.querySelectorAll('.quiz-opt').forEach(btn=>btn.onclick=()=>{
      const i=Number(btn.dataset.i);
      closeModal();
      if(i===q.correcta){
        chest.opened=true; clueState.found++; clueState.texts.push(chest.clue);
        score+=80; combo++;
        addMessage('💡 ¡PISTA ENCONTRADA!',chest.gx,chest.gy,'#ffd94d');
        spawnParticles(chest.gx*CELL+CELL/2,chest.gy*CELL+CELL/2,'#ffd94d',24);
        updateHUD();
      } else {
        score=Math.max(0,score-15); mistakes++;
        addMessage('❌ El cofre sigue cerrado',chest.gx,chest.gy,'#ff5c5c');
        updateHUD();
      }
    });
  }
  ...
  function triggerHazard(hazard) {
    if (invulnerableMs > 0 || hazard.triggered) return;
    hazard.triggered = true;
    hazard.active = false;
    releaseCage(hazard.cageId);
    score += 15;
    addMessage('⚠️ ¡TRAMPA ACTIVADA! UNA JAULA SE ABRE', hazard.gx, hazard.gy, '#ffb44d');
    spawnParticles(hazard.gx * CELL + CELL / 2, hazard.gy * CELL + CELL / 2, '#ffb44d', 20);
    updateHUD();
  }
  ...
  function applyEnemyDamage() {
    ...
    player.px = 2 * CELL; player.py = 2 * CELL;
    player.moving = false;
  ...
  function renderPowerBar() {
    ...
    if(!bar) return;
    bar.innerHTML='<span class="power-empty">🧩 Pistas en cofres · 🚪 Resuelve el acertijo de la puerta</span>';
  }
  ...
  function updateEnemy(e, dt) {
    ...
    let dir=e.dir;
    const deltas={up:[0,-1],down:[0,1],left:[-1,0],right:[1,0]};
    const distance = Math.abs(player.gx-e.gx)+Math.abs(player.gy-e.gy);
    if(e.type==='snake' && distance <= 6 && Math.random()<0.58){
    ...
    } else if(e.type==='dementor' && Math.random()<0.22){
      const dirs=['up','down','left','right']; dir=dirs[Math.floor(Math.random()*dirs.length)];
  ...
  function drawGate(gx, gy, open) {
    ...
    if (spriteReady('door')) { ctx.globalAlpha=open?1:.92; ctx.drawImage(SPRITES.door,px,py-5,CELL,CELL+5); ctx.globalAlpha=1; }
    else { ctx.fillStyle=open?'#5b8d45':'#4b3324';ctx.fillRect(px+8,py+5,CELL-16,CELL-5); }
    if (!open) { ctx.fillStyle='rgba(0,0,0,.35)';ctx.beginPath();ctx.arc(px+CELL/2,py+CELL/2,5,0,Math.PI*2);ctx.fill(); }
  }
  ...
  function drawRock(r) {
    ...
    if (spriteReady('rock')) { ctx.drawImage(SPRITES.rock,px+1,py+1,CELL-2,CELL-2); return; }
    ctx.fillStyle='#665b53';ctx.beginPath();ctx.arc(px+CELL/2,py+CELL/2,CELL/2-3,0,Math.PI*2);ctx.fill();
  }
  ...
  function drawToken(t) {
    ...
    const cx=px+CELL/2, cy=py+CELL/2+Math.sin(Date.now()/300+t.gx)*2;
    ctx.globalAlpha=t.collected?.25:1;
    if (spriteReady('diamond')) ctx.drawImage(SPRITES.diamond,px+2,py+1,CELL-4,CELL-2);
    else { ctx.fillStyle='#31c8f3';ctx.beginPath();ctx.moveTo(cx,py+4);ctx.lineTo(px+CELL-5,cy);ctx.lineTo(cx,py+CELL-4);ctx.lineTo(px+5,cy);ctx.closePath();ctx.fill(); }
    ctx.globalAlpha=1;
  }

  function drawHazard(h) {
    const px=h.gx*CELL, py=h.gy*CELL;
    if (spriteReady('trap')) { ctx.globalAlpha=h.triggered?.38:1; ctx.drawImage(SPRITES.trap,px,py,CELL,CELL); ctx.globalAlpha=1; return; }
    ctx.fillStyle='#666';ctx.fillRect(px+5,py+CELL-10,CELL-10,7);
  }
  ...
  function drawChest(c) {
    ...
    if (spriteReady('chest')) { ctx.globalAlpha=c.opened?.45:1; ctx.drawImage(SPRITES.chest,px,py,CELL,CELL); ctx.globalAlpha=1; return; }
    ctx.fillStyle='#8d5227';ctx.fillRect(px+5,py+10,CELL-10,CELL-14);
  }
  function drawPuzzleSwitch(s) {
    ...
    ctx.fillStyle=s.active?'#5b8d45':'#5b4938';ctx.fillRect(px+5,py+5,CELL-10,CELL-10);
    ctx.strokeStyle=s.active?'#b9f17c':'#d1a65d';ctx.lineWidth=2;ctx.strokeRect(px+6,py+6,CELL-12,CELL-12);
    ctx.fillStyle=s.active?'#eaffbf':'#ffd94d';ctx.font='bold 16px Georgia';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(s.active?'✓':'🔒',cx,cy);
  }
  ...
  function drawEnemy(e) {
    ...
    const sprite=e.type==='snake'?'snake':'dementor';
    if (spriteReady(sprite)) { ctx.globalAlpha=e.caged?.9:1; ctx.drawImage(SPRITES[sprite],px,py,CELL,CELL); ctx.globalAlpha=1; }
    if (e.caged) {
      if (spriteReady('cage')) ctx.drawImage(SPRITES.cage,px,py,CELL,CELL);
      else { ctx.strokeStyle='#b8b8b8';ctx.lineWidth=2;for(let i=-1;i<=1;i++){ctx.beginPath();ctx.moveTo(px+CELL/2+i*7,py+3);ctx.lineTo(px+CELL/2+i*7,py+CELL-3);ctx.stroke();} }
      ctx.fillStyle='#ffd94d';ctx.font='bold 9px system-ui';ctx.textAlign='center';ctx.fillText('⚠',px+CELL/2,py+5);
    }
  }
  function drawPlayer() {
    const px=player.px, py=player.py;
    if (invulnerableMs>0 && Math.floor(Date.now()/100)%2===0) return;
    if (spriteReady('estratega')) { ctx.drawImage(SPRITES.estratega,px,py-4,CELL,CELL+4); return; }
    const cx=px+CELL/2,cy=py+CELL/2;ctx.fillStyle='#e3b37b';ctx.beginPath();ctx.arc(cx,cy-6,8,0,Math.PI*2);ctx.fill();ctx.fillStyle='#3d668f';ctx.fillRect(cx-8,cy+2,16,12);
  }
