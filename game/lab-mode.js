...
(function () {
  ...
  // Cada estudiante tiene su propia partida local. Esto evita mezclar
  // progreso, puntajes o niveles desbloqueados al cambiar de cuenta.
  function studentSuffix() {
    const email = (student && student.email) ? student.email.toLowerCase().trim() : 'anonimo';
    return encodeURIComponent(email).replace(/%/g, '_');
  }
  function studentKey(base) { return base + '_' + studentSuffix(); }
  function readStudentObject(base) {
    try { return JSON.parse(localStorage.getItem(studentKey(base)) || '{}'); } catch (_) { return {}; }
  }
  function writeStudentObject(base, obj) {
    try { localStorage.setItem(studentKey(base), JSON.stringify(obj)); } catch (_) {}
  }
  function getBestScores() { return readStudentObject('cun_lab_best'); }
  function getProgress() { return readStudentObject('cun_lab_progress'); }
  function getStars() { return readStudentObject('cun_lab_stars'); }
  ...
  window.LabMode = {
    async init(config, levels, studentData) {
      ...
      levelsData = levels;
      student = studentData;
      // Pasar la config de poderes y personaje al motor
      cfg._levelsConfig = { poderes: levelsData.poderes || {}, personaje: levelsData.personaje || null, juego: cfg.juego || {} };
      renderLevelSelector();
      bindPdfButton();
    },
    startLevel(nivelNum) {
  ...
  function markLevelCompleted(nivelNum) {
    const p = getProgress();
    p[nivelNum] = true;
    writeStudentObject('cun_lab_progress', p);
  }
  ...
  function saveStars(nivelNum, stars) {
    const st = getStars();
    st[nivelNum] = Math.max(Number(st[nivelNum] || 0), Number(stars || 0));
    writeStudentObject('cun_lab_stars', st);
  }

  function getLevelStars(nivelNum) {
    return Number(getStars()[nivelNum] || 0);
  }
  ...
  function renderLevelSelector() {
    ...
    levelsData.niveles.forEach(n => {
      ...
      const stars = getLevelStars(n.num);
      ...
      btn.innerHTML = `
        ...
        <div class="lab-level-title">${escapeHtml(n.titulo)}</div>
        <div class="lab-level-section">📄 ${escapeHtml(n.seccion_documento)}</div>
        <div class="lab-stars" aria-label="${stars} de 3 estrellas">${starText}</div>
  ...
  function updateProgressDashboard() {
    const completed = levelsData.niveles.filter(n => isLevelCompleted(n.num)).length;
    const stars = levelsData.niveles.reduce((sum,n) => sum + getLevelStars(n.num), 0);
    const next = Math.min(9, completed + 1);
    const pct = Math.round((completed / 9) * 100);
    const progress = $('#armor-progress');
    if (progress) progress.style.width = pct + '%';
    if ($('#armor-count')) $('#armor-count').textContent = completed + '/9';
    if ($('#armor-status')) {
      $('#armor-status').textContent = completed >= 9 ? '👑 ¡Armadura destruida!' :
        completed >= 6 ? '🔥 La armadura casi está rota' :
        completed >= 3 ? '⚔️ Ya empiezan a aparecer grietas' : 'Aún está completa';
    }
    if ($('#lab-progress-levels')) $('#lab-progress-levels').textContent = completed + '/9';
    if ($('#lab-progress-stars')) $('#lab-progress-stars').textContent = stars + '/27';
    if ($('#lab-progress-next')) $('#lab-progress-next').textContent = completed >= 9 ? '🏆 Final' : 'Nivel ' + next;
    renderAchievements(completed, stars);
  }

  function renderAchievements(completed, stars) {
    const el = $('#achievement-list');
    if (!el) return;
    const best = getBestScores();
    const iaStrong = Number(localStorage.getItem(studentKey('cun_lab_ia8')) || 0) > 0;
    const unlocked = new Set();
    if (completed >= 3) unlocked.add('🧠');
    if (completed >= 4) unlocked.add('🔎');
    if (completed >= 6) unlocked.add('🎯');
    if (iaStrong) unlocked.add('📊');
    if (completed >= 9) unlocked.add('💎');
    if (completed >= 9) unlocked.add('👑');
    const items = [
      ['🧠','Cerebro estratégico','Completa 3 niveles'],
      ['🔎','Detective de mercados','Completa el nivel 4'],
      ['🎯','Misión cumplida','Completa el nivel 6'],
      ['📊','Analista','Obtén 8/10 o más en IA'],
      ['💎','Ventaja difícil de copiar','Completa el nivel 9'],
      ['👑','Sin armadura','Completa los 9 niveles']
    ];
    el.innerHTML = items.map(([icon,name,crit]) => `
      <div class="achievement ${unlocked.has(icon) ? 'unlocked' : ''}">
        <span>${unlocked.has(icon) ? icon : '🔒'}</span><div><b>${name}</b><small>${crit}</small></div>
      </div>`).join('');
  }
  ...
  function showIntro() {
    ...
    $('#lab-intro-consigna').textContent = 'Objetivo del nivel: resuelve el rompecabezas, consigue las pistas de los cofres y abre la puerta final.';
    // Narrativa del emprendedor
    const narrEl = $('#lab-intro-narrativa');
    if (narrEl) {
      if (currentLevel.narrativa) {
        narrEl.style.display = 'block';
        narrEl.querySelector('.narr-escena').textContent = currentLevel.narrativa.escena || '';
        narrEl.querySelector('.narr-objetivo').textContent = currentLevel.narrativa.objetivo || '';
      } else {
  ...
  function onGameEnd(score, duration, completed) {
    ...
    // Durante los niveles solo hay preguntas dentro del juego.
    // La evaluación abierta/IA se reserva para el final del recorrido.
    if (completed && currentLevel.num < 9) {
      const next = levelsData.niveles.find(n => n.num === currentLevel.num + 1);
      if (next) {
        currentLevel = next;
        renderLevelSelector();
    ...
    // Solo el Nivel 9 conduce a la evaluación abierta.
    showOpenQuestion();
  }
  ...
  function saveLabScore(nivelNum, score) {
    try {
      ...
      if (!b[nivelNum] || score > b[nivelNum]) {
        b[nivelNum] = score;
        writeStudentObject('cun_lab_best', b);
      }
  ...
  async function submitAnswer() {
    ...
    try {
      const answers = readStudentObject('cun_lab_answers');
      answers[currentLevel.num] = { nivel: currentLevel.num, respuesta, puntaje_juego: gameScore, duracion: gameDuration, updated: new Date().toISOString() };
      writeStudentObject('cun_lab_answers', answers);
    } catch (_) {}
    document.querySelectorAll('.screen').forEach(s => s.classList.remove('active'));
    ...
    const fbResult = await postToSheet({
      ...
      requisitos: 'Mínimo ' + currentLevel.pregunta_abierta.min_palabras + ' palabras. ' + rubricForLevel(currentLevel),
      respuesta: respuesta
    });
  ...
  function rubricForLevel(level) {
    const rubrics = {
      1: 'Evalúa: precisión de la idea (30%), actividad económica y oferta (20%), al menos 3 factores únicos/difíciles de imitar (35%), claridad (15%).',
      2: 'Evalúa: problema o necesidad real (35%), evidencia o dato verificable (20%), innovación/diferenciación (30%), coherencia (15%).',
      3: 'Evalúa: variables demográficas (25%), psicográficas (25%), comportamiento (25%), justificación del segmento (25%).',
      4: 'Evalúa: tendencia concreta (30%), fuente verificable (25%), interpretación del consumidor (25%), implicación para el negocio (20%).',
      5: 'Evalúa: portafolio claro (30%), coherencia con cliente (25%), propuesta de valor (30%), diferenciación (15%).',
      6: 'Evalúa: verbo/acción (25%), cliente y necesidad (25%), diferenciador (30%), claridad y concisión (20%).',
      7: 'Evalúa: visión futura (25%), dirección estratégica (25%), horizonte/alcance (20%), coherencia con misión (30%).',
      8: 'Evalúa: objetivos medibles (30%), valores (20%), indicadores o resultados (25%), alineación estratégica (25%).',
      9: 'Evalúa: ventaja competitiva difícil de copiar (30%), cadena de valor primaria (25%), actividades de apoyo (20%), síntesis ejecutiva (25%).'
    };
    return rubrics[level.num] || 'Evalúa precisión conceptual, evidencia, aplicación a la idea y claridad.';
  }
  ...
  function renderFinalScore(feedback) {
    ...
    const stars = totalNivel >= 8.5 ? 3 : (totalNivel >= 6.5 ? 2 : 1);
    saveStars(currentLevel.num, stars);
    if (iaScore >= 8) {
      try { localStorage.setItem(studentKey('cun_lab_ia8'), '1'); } catch (_) {}
    }
    const starEl = $('#lab-fb-stars');
